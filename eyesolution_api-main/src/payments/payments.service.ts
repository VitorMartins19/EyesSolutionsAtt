...
export class PayService {
  ...
  async create(data: CreatePayDTO) {
    ...
    // Verifique se o otica_id existe antes de atualizar tb_oticas
    const otica = await this.prisma.tb_oticas.findUnique({
      where: {
  ...
  async update(pagamentos_id: number, data: UpdatePutPayDTO) {
    ...
    await this.updatePlanoIdInOtica(
      updatedPayment.otica_id,
      updatedPayment.plano_id,
    );
